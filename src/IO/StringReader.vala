...
public class System.IO.StringReader : Reader 
{
    private string str;
    private int length;
    private int next = 0;
     ...
     * @exception  IOException  If an I/O error occurs
     */
    public override long Skip(long ns) throws IOException 
    {
        lock (lock) {
            EnsureOpen();
            if (next >= length)
            ...
            long n = (long)Math.fmin(length - next, ns);
            n = (long)Math.fmax(-next, n);
            next += (int)n;
            return n;
        }
    }
