...
public abstract class System.IO.InputStream : Object, Closeable
{
     ...
     */
    public abstract int ReadOne() throws IOException;
     ...
     * @see        java.io.InputStream#read()
     */
    public virtual int Read(char[] b, int off=0, int len=0) throws IOException
    {
        if (b == null) {
    ...
    public virtual long Skip(long n) throws IOException
    {
        ...
        int size = (int)Math.fminf(MAX_SKIP_BUFFER_SIZE, remaining);
        char[] skipBuffer = new char[size];
        while (remaining > 0) {
            nr = Read(skipBuffer, 0, (int)Math.fminf(size, remaining));
            if (nr < 0) {
                break;
            }
            remaining -= nr;
    ...
    {
        throw new IOException.NotSupported("mark/reset not supported");
    }
